...
// Classe principal responsável por executar os testes de desempenho e memória
// das diferentes implementações de tabelas hash

public class HashImplementation {
    public static void main(String[] args) {
        // Exibe a memória base do programa antes de começar os testes
        System.out.println("Memória base: " + memory() + " bytes");

        // ---------- GERAR CONJUNTO DE VALORES PARA OS TESTES ----------

        // Cria um vetor de 100k registros aleatorios
        int N_100K = 100_000;
        Registro[] valores_100K = FuncoesHash.generate_values(N_100K);

        // Cria um vetor de 1M registros aleatorios
        int N_1M = 1_000_000;
        Registro[] valores_1M = FuncoesHash.generate_values(N_1M);

        // Cria um vetor de 10M registros aleatorios
        int N_10M = 10_000_000;
        Registro[] valores_10M = FuncoesHash.generate_values(N_10M);

        // ---------- TESTES DE DESEMPENHO E MEMÓRIA ----------

        // Exibe a memória de ocupação fixa do programa (sem os dados)
        System.out.println("Memória de ocupação fixa: " + memory() + " bytes");
        System.out.println("\n\n");

        // Executa os testes para cada implementação de tabela hash
    ...
    // Testa a tabela hash linear com os parâmetros fornecidos
    ...
    // Função que executa os testes da tabela hash dupla
    ...
    // Testa a tabela hash dupla com os parâmetros fornecidos
    ...
    // Função que executa os testes da tabela hash encadeada
    ...
    // Testa a tabela hash encadeada com os parâmetros fornecidos
    ...
    // Função que executa os testes da tabela hash BTree
    ...
    // Testa a tabela hash BTree com os parâmetros fornecidos
    ...
    // Funções auxiliares para medir tempo de execução e uso de memória
    ...
    // Finaliza a contagem do tempo de execução e exibe o resultado
    static void finalizar() {
        long tempoTotal = System.currentTimeMillis() - tempoInicial;
        System.out.println(label + " - Tempo de execução: " + tempoTotal + "ms");
    }

    // Retorna a quantidade de memória atualmente utilizada pelo programa
    static long memory() {
...
// Classe auxiliar com funções de hash, geração de valores aleatórios e operações matemáticas
class FuncoesHash {
    static int min = 100000000; 
    static int max = 999999999;

    // Gera um vetor de n registros com códigos aleatórios entre min e max
    static Registro[] generate_values(int n) {
        ...
        // preenche o vetor com registros aleatórios
        for (int i = 0; i < n; i++) {
    ...
    static double raiz(double n) {
        double x = n;
        double y = 1;
        double precision = 1e-10; // O quao preciso queremos que seja
    ...
    // Implementação de floor para números double
    ...
    // Três funções de hash diferentes
    ...
    // Duas funções de sondagem para hash dupla, usando números primos próximos de 100K, 1M e 10M
